...
import 'MoviseResponse.dart';

// دوال مساعدة لقراءة القيم بأمان
int? safeReadInt(dynamic value) => value is int ? value : int.tryParse(value?.toString() ?? '');
double? safeReadDouble(dynamic value) => value is double ? value : double.tryParse(value?.toString() ?? '');
String? safeReadString(dynamic value) => value?.toString();
bool? safeReadBool(dynamic value) => value is bool ? value : (value.toString() == 'true' ? true : (value.toString() == 'false' ? false : null));
class MoviesResponseAdapter extends TypeAdapter<MoviesResponse> {
  ...
  MoviesResponse read(BinaryReader reader) {
    return MoviesResponse(
      status: safeReadInt(reader.read())?.toString() ?? '0',
...
class MetaAdapter extends TypeAdapter<Meta> {
  ...
  Meta read(BinaryReader reader) {
    return Meta(
      ...
      apiVersion: safeReadInt(reader.read()), // استخدم safeReadInt
      executionTime: safeReadString(reader.read()), // استخدم safeReadDouble
    );
  }
...
class DataAdapter extends TypeAdapter<Data> {
  ...
  Data read(BinaryReader reader) {
    return Data(
      ...
      limit: safeReadInt(reader.read()),
      pageNumber: safeReadInt(reader.read()),
...
class MoviesAdapter extends TypeAdapter<Movies> {
  ...
  Movies read(BinaryReader reader) {
    return Movies(
      ...
      genres: (reader.read() as List?)?.map((e) => safeReadString(e)!).toList() ?? [],
...
class TorrentsAdapter extends TypeAdapter<Torrents> {
  ...
  Torrents read(BinaryReader reader) {
    return Torrents(
      ...
      isRepack: safeReadBool(reader.read()).toString(),
